...
echo "Task ${TASK_ID} (slurm task id=${SLURM_ARRAY_TASK_ID}) running on node ${SLURM_NODENAME} with GPU ${CUDA_VISIBLE_DEVICES}"
echo "PROCESSING: $MAKEFILE_TARGET with UUID_PATH: $UUID_PATH"
echo "DATADIR: $DATADIR"

echo -e "$(date +"%Y-%m-%dT%T") START (${TASK_ID}): $UUID_PATH" >> logs/PressMint-CZ.slurm.log
...
RES=$(/usr/bin/time --output=logs/PressMint-CZ.slurm.$SLURM_JOB_ID.${TASK_ID}.tmp -f "%x#%E real, %U user, %S sys, %M kB" $CMD)
TIME=$(cut -d'#' -f 2 logs/PressMint-CZ.slurm.$SLURM_JOB_ID.${TASK_ID}.tmp)
CODE=$(cut -d'#' -f 1 logs/PressMint-CZ.slurm.$SLURM_JOB_ID.${TASK_ID}.tmp)
if [ $((10#$CODE)) -gt 0 ]; then STATUS="FAILED-$CODE"; else STATUS="FINISHED"; fi
rm logs/PressMint-CZ.slurm.$SLURM_JOB_ID.${TASK_ID}.tmp
echo -e "$(date +"%Y-%m-%dT%T") DONE (${TASK_ID}): $UUID_PATH   $STATUS    $SLURM_JOB_ID/${TASK_ID}   $(hostname)     $TIME         $CMD" >> logs/PressMint-CZ.slurm.log
